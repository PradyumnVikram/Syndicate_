...
def agent_v0(domain_goal: str, domain_tools: list[dict], task_input: dict[str, Any],
             seed: int = 42) -> tuple[str, list[dict]]:
    """Run the domain-parametric ReAct/CoT agent.

    Args:
        domain_goal: The goal string from TaskDomain.goal
        domain_tools: List of tool spec dicts (name, json_schema)
    ...
    messages = [
        {"role": "system", "content": system_prompt},
        {
            ...
            "content": (
                f"Goal: {domain_goal}\n\n"
                f"Available tools:\n"
                f"{json.dumps(domain_tools, indent=2)}\n\n"
    ...
    response = deterministic(messages, tools=domain_tools, seed=seed)
    traces.append({
        "step": 1,
        "module": "tool_call",
        "inputs": {"messages": messages, "tools": domain_tools},
        "outputs": response,
    })
    ...
    final_response = deterministic(
        final_messages, tools=domain_tools, seed=seed + 2
    )
    traces.append({
        "step": 3,
        "module": "thought",
...
if __name__ == "__main__":
    ...
    answer, traces = agent_v0(
        ...
        domain_tools=[t.json_schema for t in domain.tools],
    ...
    result_real = broker.handle_call({
        ...
        'tier': 'deterministic',
        'messages': [
            {'role': 'system', 'content': 'You are a calculator. Answer briefly.'},
            {'role': 'user', 'content': 'What is 2+2?'}
        ],
        'tools': [{'name': 'calculator', 'json_schema': calculator_json_schema}],
        'seed': 42,
        'max_tokens': 500,
    })
